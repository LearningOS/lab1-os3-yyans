...
impl TaskManager {
    ...
    fn get_task_info(&self) -> TaskInfo {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let current_task = inner.tasks[current].borrow();
        TaskInfo {
            status: TaskStatus::Running,
            syscall_times: current_task.syscall_times.as_slice().try_into().unwrap(),
            time: (get_time_us() - current_task.start_time) / 1000,
        }
    }
}
